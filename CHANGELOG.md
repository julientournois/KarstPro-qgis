...
### Ajouté
- **Export MLL : CSV de données par doline** (`mll_data_<secteur>_<date>.csv`,
...
- **Blocage de 8h+ à la préparation de grands secteurs boisés** (Poligny,
  Chaux-Neuve). Cause : `shafts.py` déclasse automatiquement en
  `interet=0` tout gouffre détecté sous canopée fermée (faux positifs
  ...
  (`shapely.strtree.STRtree`) dans `apply_gouffres_carryover` et
